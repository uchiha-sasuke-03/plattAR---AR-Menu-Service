...
async function main() {
    ...
    const run = async (command, args, environmentVariables = {}, workingDirectory = process.cwd()) => new Promise((resolve, reject) => {
    ...
    const updateScreenshots = async (config) => {
        ...
        for (const renderer of renderers) {
            const { name: rendererName, description: rendererDescription, scripts: scripts } = renderer;
            const screenshotCreator = new ArtifactCreator(config, rootDirectory, `http://localhost:${args.port}/packages/render-fidelity-tools/test/renderers/${rendererName}/`);
            for (const scenarioBase of scenarios) {
                ...
                try {
                    await screenshotCreator.captureScreenshot(rendererName, scenarioName, dimensions, goldenPath, -1, args.quiet);
                }
                catch (error) {
                    throw new Error(`Failed to update ${rendererDescription} screenshot: ${error.message}`);
                }
            }
...
main();
//# sourceMappingURL=render-goldens.js.map