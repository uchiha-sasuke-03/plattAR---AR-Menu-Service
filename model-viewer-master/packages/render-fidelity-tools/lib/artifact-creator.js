...
import { DEVICE_PIXEL_RATIO, FIDELITY_TEST_THRESHOLD, ImageComparator, toDecibel } from './common.js';
...
export class ArtifactCreator {
    ...
    get goldens() {
        return this.config.renderers.map(renderer => ({ ...renderer, file: `${renderer.name}-golden.png` }));
    }
    async compareRenderers(scenario, renderer) {
        ...
        console.log(`Start to compare ${renderer}'s golden with other renderers' goldens:`);
        const rendererIndex = 0;
        const rendererGoldenPath = join(rootDirectory, 'goldens', scenarioName, goldens[rendererIndex].file);
        let rendererGolden;
        try {
            rendererGolden = await fs.readFile(rendererGoldenPath);
        }
        catch (error) {
            throw new Error(`❌ Failed to read ${renderer}'s ${scenarioName} golden! Error message: ${error.message}`);
        }
        // save goldens images to result folder which will be used to show in the
        // result-viewer page.
        await fs.writeFile(join(outputDirectory, scenarioName, goldens[rendererIndex].file), rendererGolden);
        const rendererGoldenImage = new Uint8ClampedArray(pngjs.PNG.sync.read(rendererGolden).data);
        for (const golden of goldens) {
            ...
            const candidateGoldenPath = join(rootDirectory, 'goldens', scenarioName, golden.file);
            let candidateGolden;
            try {
                candidateGolden = await fs.readFile(candidateGoldenPath);
            }
            catch (error) {
                throw new Error(`❌ Failed to read ${golden.name}'s ${scenarioName} golden! Error message: ${error.message}`);
            }
            // save goldens images to result folder which will be used to show in the
            // result-viewer page.
            await fs.writeFile(join(outputDirectory, scenarioName, golden.file), candidateGolden);
            const candidateGoldenImage = new Uint8ClampedArray(pngjs.PNG.sync.read(candidateGolden).data);
            const analysisResult = await this.analyze(rendererGoldenImage, candidateGoldenImage, dimensions);
            analysisResults.push(analysisResult);
        }
        const scenarioRecord = { analysisResults, scenario };
        console.log(`\n💾 Recording analysis`);
        await fs.writeFile(join(outputDirectory, scenarioName, 'analysis.json'), JSON.stringify(scenarioRecord));
    }
    async captureAndAnalyzeScreenshot(scenario, renderer, quiet = false) {
        const { rootDirectory, goldens } = this;
        const { name: scenarioName, dimensions, exclude } = scenario;
        console.log(`start compare ${renderer}'s golden with ${renderer}'s screenshot generated from fidelity test:`);
        let screenshot;
        try {
            // set the output path to an empty string to tell puppeteer to not save
            // the screenshot image
            screenshot = await this.captureScreenshot(renderer, scenarioName, dimensions, '', 60, quiet);
        }
        catch (error) {
            if (this.browser != null) {
                await this.browser.close();
            ...
            throw new Error(`❌ Failed to capture ${renderer}'s screenshot of ${scenarioName}. Error message: ${error.message}`);
        }
        if (screenshot == null) {
            throw new Error(`❌ ${renderer}'s screenshot of ${scenarioName} is not captured correctly (value is null).`);
        }
        const buffer = Buffer.from(screenshot);
        const screenshotImage = new Uint8ClampedArray(pngjs.PNG.sync.read(buffer).data);
        const rendererIndex = 0;
        const rendererGoldenPath = join(rootDirectory, 'goldens', scenarioName, goldens[rendererIndex].file);
        let rendererGolden;
        try {
        ...
        const rendererGoldenImage = new Uint8ClampedArray(pngjs.PNG.sync.read(rendererGolden).data);
        const result = await this.analyze(screenshotImage, rendererGoldenImage, dimensions);
        ...
        if (rmsInDb > FIDELITY_TEST_THRESHOLD) {
            if (exclude?.includes(renderer)) {
                console.log(`❌ Skipped! Senario name: ${scenario.name}, rms distance ratio: ${rmsInDb.toFixed(2)} dB.`);
            }
            else {
                throw new Error(`❌ Senarios name: ${scenario.name}, rms distance ratio: ${rmsInDb.toFixed(2)} dB.`);
            }
        }
        return result;
    }
    async fidelityTest(scenarioWhitelist = null, renderer, dryRun = false, quiet = false) {
        const { scenarios } = this.config;
        const { outputDirectory } = this;
        const analyzedScenarios = [];
        const fidelityRegressionResults = { results: [], errors: [], warnings: [] };
        const compareRenderersErrors = [];
        for (const scenarioBase of scenarios) {
            ...
            try {
                ...
                const errorMessage = `❌Fail to compare ${renderer} with other renderers of scenario ${scenarioName}. Error message: ${error.message}`;
            ...
            if (!dryRun) {
                try {
                    ...
                    const message = `❌Fail to analyze scenario :${scenarioName}! Error message: ${error.message}`;
        ...
        const finalConfig = Object.assign({}, this.config, { scenarios: analyzedScenarios, errors: compareRenderersErrors });
        // ensure directory exists, this can happen if all scenarios are ignored
        await fs.mkdir(outputDirectory, { recursive: true });
        await fs.writeFile(join(outputDirectory, 'config.json'), JSON.stringify(finalConfig));
        await fs.writeFile(join(outputDirectory, 'fidelityRegressionResults.json'), JSON.stringify(fidelityRegressionResults));
    }
    async analyze(candidateImage, goldenImage, dimensions) {
        const imageDimensions = {
            width: dimensions.width * DEVICE_PIXEL_RATIO,
            height: dimensions.height * DEVICE_PIXEL_RATIO
        };
        const comparator = new ImageComparator(candidateImage, goldenImage, imageDimensions);
        const { analysis } = comparator.analyze();
        const { rmsDistanceRatio } = analysis;
        console.log(`\n  📊 Decibels of root mean square color distance: ${(10 * Math.log10(rmsDistanceRatio)).toFixed(2)}`);
        return analysis;
    }
    async captureScreenshot(renderer, scenarioName, dimensions, outputPath = join(this.outputDirectory, 'model-viewer.png'), maxTimeInSec = -1, quiet = false) {
        ...
        const screenshot = await page.screenshot({ path: outputPath, omitBackground: true });
        page.close();
        return screenshot;
    }
}
